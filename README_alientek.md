...
该 BOX 作为独立菜单设备，联网后进入本地菜单待机，不触发原小智云激活或原云 OTA。私有构建使用下面的自有 OTA 通道。此行为仅作用于第一代 BOX，其余板型保持原实现。亮度、音量和 K0 背光开关功能保留。
...
服务入口：https://ota.ainotex.com ，香港服务器提供 HTTPS。BOX 联网约 5 秒后检查一次，此后每 5 分钟检查；请求或更新失败后约 60 秒重试。只有更高版本才下载，成功后自动重启。亮度、音量和已保存 Wi-Fi 保留。

设备使用 `/box/latest.json` 清单和 `/box/firmware/<版本>.bin` 应用镜像；下载需要 Bearer 凭据。凭据头 `box_ota_private.h` 放在 Git 之外，内容为 `#define BOX_OTA_TOKEN "<私有凭据>"`，构建时用环境变量 `BOX_OTA_PRIVATE_DIR` 指向所在目录。没有私有头的公开构建不启动此更新任务。不要将凭据、Apple 字体或含这些资源的固件提交到公开仓库。

清单格式：

```json
{"board":"atk-dnesp32s3-box","version":"2.4.4","url":"https://ota.ainotex.com/box/firmware/2.4.4.bin","size":1234567,"sha256":"64位小写十六进制摘要"}
```

设备严格校验板型、三段数字版本、下载域名与路径、长度、SHA-256 及 ESP 应用镜像，写入另一 OTA 分区；验证完成后才切换启动分区。更新期间界面显示进度并暂停 K1/K2 导航，失败后恢复菜单。此通道只分发 `xiaozhi.bin` 应用镜像，不修改分区表、资源分区或 NVS；硬件型号必须保持为第一代 BOX。

发布新版本：递增根 `CMakeLists.txt` 的 `PROJECT_VER`（也可通过 `BOX_FIRMWARE_VERSION` 环境变量设置），用相同板型和私有资源构建，然后执行：

```sh
python3 scripts/publish_box_ota.py --firmware /path/to/xiaozhi.bin --version 2.4.4 --identity /path/to/ssh-key
```

脚本通过 SSH 上传到 `/var/www/esp32s3-ota/box`，核对应用版本与远端 SHA-256，然后原子替换清单。已发布版本不能以不同内容覆盖。服务配置位于香港服务器 `/etc/nginx/conf.d/ota-ainotex.conf`（含私有凭据，不入 Git）；证书由 Certbot 自动续期。网站首页公开，固件与清单限制为授权设备访问。

2026-09-09 验证：60 项主机测试通过，公开 Noto 字体构建和私有 Apple 字体构建均成功。实机从 USB 引导版 `2.4.3`（ota_0）经 HTTPS 下载 `2.4.4`，SHA-256 校验通过，自动切换到 ota_1 并重启；再次联网确认版本已最新，亮度 50、音量 20 和 Wi-Fi 配置保留。


### 2.4.5 显示优化

- 保持原生 320×240 渲染和现有 4bpp 抗锯齿字体；提高辅助文字对比度。
- 主菜单图标及进入箭头使用整数像素坐标的原生图形，不放大小位图。
- 翻页采用 240ms 缓出横移，移除卡片整体透明混合，减少移动中文字变淡和中间合成开销；减轻阴影、增加细边框。
- 第一代 BOX 使用两块 32 行 DMA 绘制缓冲（合计 40 KiB），允许绘制与传输交叠；原先为单块 20 行缓冲（12.5 KiB）。其他调用保留原默认参数。
- BOX 构建的 LVGL 刷新周期设为 16ms，接近 60 次/秒调度；这不是实测动画帧率。LCD 并口仍为 10MHz，没有调整面板扫描频率。

实机已从 `2.4.4` 经自有 OTA 升级至 `2.4.5` 并恢复 Wi-Fi、亮度和音量。用户确认：主菜单及设置页显示完整，连续翻页、滑动和按键正常。60 项主机测试通过，BOX0 兼容构建通过（仅编译，未向 BOX 设备烧录）。


### 2.4.6 离线音乐

第 1 个主功能为“音乐”，其余测试页面和第 5 项设置保留。当前歌单依次为 Apashe 的 Lacrimosa、Flux Pavilion 的 Pull The Trigger（Maduk Remix）及 Razihel & Aero Chord 的 Titans。

| 按键 | 音乐页操作 |
| --- | --- |
| 短按 K2 / K1 | 上一首 / 下一首；播放中切歌会继续播放 |
| 长按 K1 | 播放 / 暂停；暂停后可继续 |
| 长按 K2 | 停止并返回主菜单 |
| 短按 K0 | 熄屏 / 亮屏，保持播放 |

歌曲结束后自动播放下一首，三首循环；音量使用设置页保存值。页面显示曲名、艺人、曲目编号和时间进度。播放在独立工作任务进行，使用既有 Opus 解码及音频输出；暂停恢复按 60ms 音频帧定位。

歌曲从用户本地 MP3 转为 24kHz 单声道、64kbps Opus，每包 60ms。使用 BXM1 偏移索引支持暂停定位，作为 `box_music_0.bxm` 至 `box_music_2.bxm` 保存在独立 assets 资源分区中，无需联网或 SD 卡。音源及含歌曲的资源镜像不进入 Git，也不上传公开固件下载区。

首次装入歌曲应先备份设备当前 assets 分区，再调用 `scripts/pack_box_music.py --base <备份镜像> --output <新镜像> <歌曲1.mp3> <歌曲2.mp3> <歌曲3.mp3>`。工具检查原资源校验和，保留现有文件与尺寸信息，拒绝超过 8 MiB 的结果。将新资源镜像写入原 assets 地址 `0x800000`；不要擦除 NVS 或更改分区表。后续应用 OTA 不改写歌曲分区；重新刷入默认生成的 assets 镜像会移除歌曲，应保留私有资源备份。

验证记录：62 项主机测试通过，公开 Noto 字体和私有 Apple 字体构建均成功。资源镜像 7,328,239 字节，原有 24 个资源逐项比对保持一致，新增 3 个音乐资源；设备写入校验通过。实机经 OTA 启动 `2.4.6`，识别三首歌曲，时长约 3:37、3:40、4:10；已观察到播放和停止控制事件。
