...
class VersionTests(unittest.TestCase):
    ...
    def test_current_matrix_contains_only_alientek_esp32s3(self):
        configs = list((ROOT / "main/boards").rglob("config.json"))
        self.assertEqual(len(configs), 8)
        for path in configs:
            config = json.loads(path.read_text())
            self.assertEqual(config["manufacturer"], "alientek")
            self.assertEqual(config["target"], "esp32s3")
        for version in ((6, 0, 2), (6, 1, 0)):
            variants = build._collect_variants(idf_version=version)
            self.assertEqual(len(variants), 6)  # S3M entries have no upstream release builds.
            self.assertEqual(len({v["full_name"] for v in variants}), 6)
            self.assertTrue(all(v["board"].startswith("alientek/") for v in variants))
            self.assertTrue(all(v["target"] == "esp32s3" for v in variants))
    ...
    def test_chip_defaults_only_contain_target_overrides(self):
        ...
        expected_partitions = {"esp32s3": '"partitions/v2/16m.csv"'}
...
class BoardMenuTests(unittest.TestCase):
    ...
    def test_board_menu_has_explicit_target_defaults(self):
        choice = build._kconfig_choice("BOARD_TYPE")
        self.assertEqual(choice["default"], "BOARD_TYPE_ATK_DNESP32S3")
        self.assertEqual(len(choice["entries"]), 8)
        self.assertIn("depends on IDF_TARGET_ESP32S3", choice["block"])
...
class CliTests(unittest.TestCase):
    def setUp(self):
        self.variants = [
            {
                "board": "alientek/atk-dnesp32s3",
                "name": "alientek/atk-dnesp32s3",
                "full_name": "alientek/atk-dnesp32s3",
            },
            {
                "board": "multi-board",
                "name": "variant-a",
    ...
    def test_build_does_not_create_zip_by_default(self):
        with (
            ...
            build.main(["alientek/atk-dnesp32s3"])

        build_board.assert_called_once_with(
            "alientek/atk-dnesp32s3",
            config_filename="config.json",
            name_filter="alientek/atk-dnesp32s3",
            create_zip=False,
            language=None,
            wake_word=None,
            build_options=None,
    ...
    def test_zip_flag_is_forwarded(self):
        with (
            ...
            build.main(["alientek/atk-dnesp32s3", "--zip"])
